...
class WorkflowEngine:
    ...
    def transition(transaction, target_step_id_or_action, user, remarks='', **kwargs):
        ...
        # 2. Fallback to Persona/Role-based reusable signature if the user has one (SPMO)
    ...
    def _send_notification_to_next_role(transaction, next_step):
        ...
        if recipient_emails:
            subject = f"Asset Manager Action Required: {next_step.label}"
            ...
            message = (
                ...
                f"Please log in to Asset Manager to review and process this document."
            )
            try:
