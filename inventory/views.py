...
from .masking import mask_name, mask_department, mask_username
...
def toggle_demo_mode(request):
    current = request.session.get('demo_mode', False)
    request.session['demo_mode'] = not current
    messages.success(request, f"Demo Mode {'Enabled' if not current else 'Disabled'}")
...
def dashboard(request):
    ...
    if request.user.is_superuser and not demo_role:
        ...
        # Unit Persona View: Filter by a fixed demo department (Agency A)
    ...
    context = {
        ...
        'top_office_name': mask_department(top_office_name, 1, is_demo), # Hardcoded ID 1 for top
...
def get_user_tab_permissions(user, role_override=None):
    ...
    if role in ('SPMO_ADMIN', 'SPMO_ADMIN'):
        perms['property'] = True
        perms['lifecycle'] = True
        perms['government'] = True
...
def asset_detail(request, pk):
    ...
    if request.user.is_superuser and not demo_role:
        ...
        # Unit Persona: Strict Department Isolation (Fixed to Agency B ID 128)
        from .models import Department
        try:
            demo_dept = Department.objects.get(id=128)
...
def print_par(request, pk):
    ...
    context = {
        'batch': batch,
        'par_pages': par_pages,
        'issued_by_name': "ISAAC L. BROWN",
        'issued_by_position': "Acting Chief, Institutional SPMO",
...
def print_par_v2(request, pk):
    ...
    context = {
        'batch': batch,
        'par_pages': par_pages,
        'issued_by_name': signatures.get('authorized', {}).get('name', "ISAAC L. BROWN"),
...
def rpcppe_report(request):
    ...
    if demo_role and demo_role.startswith('UNIT_'):
        # Unit Persona: Strict Department Isolation (Fixed to Agency A ID 127)
        from .models import Department
        try:
            demo_dept = Department.objects.get(id=128)
...
def guest_login(request):
    """Auto-logs in a guest user for demo friction reduction."""
    from django.contrib.auth import login as auth_login
    from django.contrib.auth.models import User
    
    # Use environment variable to toggle this
    if os.environ.get('ALLOW_GUEST_ACCESS', 'True').lower() != 'true':
         messages.error(request, "Guest access is disabled for this instance.")
    ...
    auth_login(request, guest_user, backend='django.contrib.auth.backends.ModelBackend')
    
    # Set a demo role in session
    request.session['active_demo_role'] = 'UNIT_Agency A' 
    
    messages.success(request, "Welcome! You are browsing as a Guest.")
    return redirect('dashboard')
